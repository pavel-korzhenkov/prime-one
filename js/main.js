...
function formValidate(input){
	...
	if(input.attr('name')=='emailphone' || input.hasClass('emailphone')){
		if(input.val()!=input.attr('data-value')){
			var em=input.val().replace(" ","");
			input.val(em);
		}
		if(!((/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,8})+$/.test(input.val())) || (/^\d[\d\(\)\ -]{4,14}\d$/.test(input.val()))) || input.val()==input.attr('data-value')){
				er++;
			addError(input);
		}else{
			removeError(input);
	...
	if(input.hasClass('name')){
		if(!((/^[А-Яа-яa-zA-Z-]+( [А-Яа-яa-zA-Z-]+)$/.test(input.val())) || (/^[А-Яа-яa-zA-Z-]+$/.test(input.val())))){
				er++;
			addError(input);
		}
	}
